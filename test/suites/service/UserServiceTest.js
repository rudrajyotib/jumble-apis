...
describe("should operate user operations", function () {
    ...
    it("getFriendshipDetailsForChallenge: should assert friend is challengeable and add scores if status is open", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().resolves({
            found: true, data: {
                status: 'open',
                sourceUserId: 'someOtherSourceUserId',
                score: { someSourceUserId: 1, someTargetUserId: 2 }
            }
        })
        const result = await userService.getFriendshipDetailsForChallenge('someSourceUserId', 'someTargetUserId')
        userRepoFriendSearchExpectation.verify()
        challengeRepoDuelSearchExpectation.verify()
        sinon.assert.calledWith(userRepoFriendSearchExpectation.getCall(0), sinon.match('someSourceUserId'), sinon.match('someTargetUserId'))
        sinon.assert.calledWith(challengeRepoDuelSearchExpectation.getCall(0), sinon.match('someDuelId'))
        assert.isTrue(result.isFriend)
        assert.isTrue(result.challengeable)
        assert.equal(1, result.sourceUserScore)
        assert.equal(2, result.targetUserScore)
    })

    it("getFriendshipDetailsForChallenge: should assert friend is challengeable if status is active and source user ID matches with duel source", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().resolves({ found: true, data: { status: 'active', sourceUserId: 'someSourceUserId', score: { someSourceUserId: 1, someTargetUserId: 2 } } })
        const result = await userService.getFriendshipDetailsForChallenge('someSourceUserId', 'someTargetUserId')
        userRepoFriendSearchExpectation.verify()
        challengeRepoDuelSearchExpectation.verify()
        sinon.assert.calledWith(userRepoFriendSearchExpectation.getCall(0), sinon.match('someSourceUserId'), sinon.match('someTargetUserId'))
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if status is active and source user ID does not match with duel source", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().resolves({ found: true, data: { status: 'active', sourceUserId: 'someTargetUserId', score: { someSourceUserId: 1, someTargetUserId: 2 } } })
        const result = await userService.getFriendshipDetailsForChallenge('someSourceUserId', 'someTargetUserId')
        userRepoFriendSearchExpectation.verify()
        challengeRepoDuelSearchExpectation.verify()
        sinon.assert.calledWith(userRepoFriendSearchExpectation.getCall(0), sinon.match('someSourceUserId'), sinon.match('someTargetUserId'))
        sinon.assert.calledWith(challengeRepoDuelSearchExpectation.getCall(0), sinon.match('someDuelId'))
        assert.isTrue(result.isFriend)
        assert.isFalse(result.challengeable)
        assert.equal(1, result.sourceUserScore)
        assert.equal(2, result.targetUserScore)
    })

    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if duel is not active or open", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().resolves({ found: true, data: { status: 'pendingAction', sourceUserId: 'someSourceUserId', score: { someSourceUserId: 1, someTargetUserId: 2 } } })
        const result = await userService.getFriendshipDetailsForChallenge('someSourceUserId', 'someTargetUserId')
        userRepoFriendSearchExpectation.verify()
        challengeRepoDuelSearchExpectation.verify()
        sinon.assert.calledWith(userRepoFriendSearchExpectation.getCall(0), sinon.match('someSourceUserId'), sinon.match('someTargetUserId'))
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if duel is not found", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().resolves({ found: false })
        ...
        sinon.assert.calledWith(userRepoFriendSearchExpectation.getCall(0), sinon.match('someSourceUserId'), sinon.match('someTargetUserId'))
        sinon.assert.calledWith(challengeRepoDuelSearchExpectation.getCall(0), sinon.match('someDuelId'))
        assert.isFalse(result.isFriend)
        assert.notProperty(result, 'challengeable')
        assert.notProperty(result, 'sourceUserScore')
        assert.notProperty(result, 'targetUserScore')
    })

    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if duel search rejects", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.once().rejects({ error: 'some error' })
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if duelID is empty", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed', duelId: '' })
        challengeRepoDuelSearchExpectation.never()
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if duelID is not present", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'confirmed' })
        challengeRepoDuelSearchExpectation.never()
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if friendship is not confirmed", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: true, status: 'pending', duelId: 'someDuelId' })
        challengeRepoDuelSearchExpectation.never()
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if friendship is not foubd", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().resolves({ found: false })
        challengeRepoDuelSearchExpectation.never()
    ...
    it("getFriendshipDetailsForChallenge: should negate friend is challengeable if friendship check is rejected by repo", async function () {
        let userRepoFriendSearchExpectation = userRepositoryMock.expects('getFriendshipDetails')
        let challengeRepoDuelSearchExpectation = challengeRepositoryMock.expects('getDuel')
        userRepoFriendSearchExpectation.once().rejects({ error: 'mock error' })
        challengeRepoDuelSearchExpectation.never()
